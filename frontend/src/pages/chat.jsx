...
function Chat({ activeFriend: propActiveFriend }) {
  ...
  const fetchFriendData = async () => {
    try {
      const [friendsRes, sentRes, receivedRes] = await Promise.all([
        ...
        axios.get(`http://localhost:5000/api/friendrequest/sent/${userId}`),
        axios.get(`http://localhost:5000/api/friendrequest/received/${userId}`)
      ]);
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:5000/api/chat/${userId}/${activeFriend}`)
      .then(res => {
  ...
  const sendMessage = async () => {
    ...
    try {
      await axios.post(`http://localhost:5000/api/chat/${userId}/${activeFriend}`,
        { text: input },
          ...
          {activeFriend ? (
            activeFriend === "bot" ? chat.map((msg, i) => (
              <div key={i} className={`max-w-md ${msg.from === 'user' ? 'ml-auto text-right' : ''}`}>
                <p className="text-xs text-gray-500 mb-1">{msg.from === 'user' ? 'You' : 'Bot'}</p>
